...
class ImageStack(object):
    def __init__(self, h5filename, image_type='image', freq=None, steps=None, mode='r'):
        ...
        if self.beam and 'SIGMA' in self.group['beam'].attrs:
            self.sigma = self.group['beam'].attrs['SIGMA']
        else:
            self.sigma = np.array((1.0, 1.0))
        if self.beam and self.group.keys() and 'SCALE' in self.group['beam'].attrs:
    ...
    def pix2ts(self, x, y, avg_pol=True, correct=True):
        ...
        """
        ts = self.data[:, y, x, self.channel, self.steps[0]:self.steps[1]].astype(np.float64)
        if avg_pol is True and self.n_pol>1:
